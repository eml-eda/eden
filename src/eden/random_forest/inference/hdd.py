...
FAST_MODE = True
N_JOBS = 16
ADAPTIVE_MODEL = "results/rf/adaptive/hdd-adaptive-baseline-rf.json"

fpath = "results/rf/gridsearch/hdd-gridsearch-rf.csv"
...
def deploy_grid(fpath_grid_search):
    ...
    # models.sort_values(by=["n_branches_val"], ascending=True, inplace=True)

    # (X_train, X_val, X_test, y_train, y_val, y_test, _,) = prepare_hdd(
    #    bits_input=32,
    # )
    models = pareto(
        models,
    ...
    for model in [*models.itertuples()][::-1]:
        ...
        X_test = deployment_subset(classifier, X_test)
    ...
    output_fname_json = "hdd-adaptive-baseline-rf.json"
...
def deploy_adaptive():
    ...
    for policy in ["aggregated-max", "qwyc"]:
        ...
        with open("hdd-adaptive-baseline-rf-deployed.json", "w") as f:
