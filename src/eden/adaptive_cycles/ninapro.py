...
top_model_gbt = json.load(
    open("results/gbdt/adaptive/ninapro-adaptive-baseline-deployed.json")
)
top_model_rf = json.load(
    open("results/rf/adaptive/ninapro-adaptive-baseline-rf-deployed.json")
)
model_rf = deepcopy(top_model_rf)
for k in model_rf:
    model_rf[k].pop("adaptive-profiling")


model_gbt = deepcopy(top_model_gbt)
for k in model_gbt:
    model_gbt[k].pop("adaptive-profiling")
...
def benchmark_gbdt(fpath):
    ...
    for patient in range(1, 27 + 1):
        ...
        results = classifier.adaptive_benchmark(
            ...
            cycles_data=top_model["adaptive-profiling"],
        )
        results = results.assign(**model_gbt[str(patient)])
...
def benchmark_rf(fpath):
    ...
    for patient in range(1, 27 + 1):
        ...
        results = results.assign(**model_rf[str(patient)])
        results_global.append(results)
        results_csv = pd.concat(results_global)
        results_csv.to_csv(output_fname, index=False)
    return results
