...
// TREE_C_START
struct Node *current_node = ROOTS[t];
...
% if leaf_store_mode == "index":
  prediction = LEAVES[current_node->right_child];
% elif leaf_store_mode == "value": 
  prediction = *(current_node->threshold);
% endif
#if N_CORES>1
