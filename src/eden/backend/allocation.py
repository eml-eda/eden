...
def _cache_placer(*, l1_bytes=64, **kwargs: Mapping[str, Tuple[int, int]]) -> Mapping:
    # TODO : Document this
    ...
    _, idx_in_sack = _knapsack_01(
        weights=weights, values=access_rates, capacity=l1_bytes
    )

    buffer_placement = {}
    for idx in range(len(buffers_to_place.keys())):
        if idx in idx_in_sack:
            buffer_placement[list(buffers_to_place.keys())[idx]] = "L1"
        else:
            buffer_placement[list(buffers_to_place.keys())[idx]] = ""
