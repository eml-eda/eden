...
START_PATIENT = 1
END_PATIENT = 27
OUTPUT_FILENAME = f"ninapro_gridsearch{START_PATIENT}to{END_PATIENT}.csv"
N_PROCESSES = 24
...
def benchmark_setup(patient, max_depth, bits_input):
    ...
    if clf is None:
        ...
        jbl.dump(
            value=clf,
            filename=f"logs/gbdt/ninapro/ninapro-patient{patient}-maxdepth{max_depth}-bitsinput{bits_input}-estimators{GBT_MAX_ESTIMATORS}.jbl",
        )
