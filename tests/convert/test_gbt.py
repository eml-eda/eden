...
def test_regression_gbt(
    ...
    golden = list()
    for row in clf.estimators_:
        preds = np.zeros_like(row, dtype=float)
        for i, e in enumerate(row):
            preds[i] = e.predict(test_data.reshape(1, -1)).reshape(-1)
        golden.append(preds)
